...
async def test_get_current_user_success(
    ...
    with patch.object(
        ...
        response = client.get(
            "/api/users/me", headers={"Authorization": f"Bearer {test_user.id}"}
        )
...
@pytest.mark.asyncio
async def test_update_current_user_not_found(
    client: TestClient, test_user: UserProfile
) -> None:
...
@pytest.mark.asyncio
async def test_list_users_success(client: TestClient, test_user: UserProfile) -> None:
