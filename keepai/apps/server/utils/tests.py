...
class TestLogger(unittest.TestCase):
    ...
    def setUp(self):
        ...
        file_handler = RotatingFileHandler(
            self.log_file, maxBytes=1024, backupCount=5  # 1KB para testes
        )
    ...
    def test_log_rotation(self):
        ...
        backup_exists = any(os.path.exists(f"{self.log_file}.{i}") for i in range(1, 6))
    ...
    def test_log_formatting(self):
        ...
        self.assertRegex(
            log_line, r"\[\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\]"
        )  # Data/hora
